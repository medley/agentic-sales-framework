"""
Approval Rules - Gates and lifecycle control for prospecting artifact approval

This module defines:
- Prospect lifecycle states
- Approval eligibility rules (for sending)
- Gate evaluation logic

Approval means: Human reviewed and approved to send outbound.
Approval does NOT imply an opportunity exists.

Approved artifacts stay in 02_Prospecting/agent-prospecting.
Nothing moves to 01_Accounts/_Active during prospecting.

Usage:
    from approval_rules import ApprovalGate, ProspectState
...
class ProspectState(str, Enum):
    ...
    # Human approved for sending (stays in 02_Prospecting)
...
@dataclass
class ApprovalEligibilityResult:
...
PromotionEligibilityResult = ApprovalEligibilityResult
...
# Warnings that block approval (cannot be forced)
BLOCKING_WARNINGS = {
    "THIN_RESEARCH",
    "VENDOR_DATA_ONLY"
}
...
class ApprovalGate:
    """
    Evaluates approval eligibility for prospecting artifacts.

    Approval means: Human reviewed and approved to send outbound.
    Approved artifacts remain in 02_Prospecting (not moved to 01_Accounts).
    ...
    def evaluate(
        ...
        """
        Evaluate whether artifacts are eligible for approval.
        ...
        Returns:
            ApprovalEligibilityResult with eligible flag and reasons
        """
        reasons = []
        warnings_present = []
        can_force = True
        ...
        return ApprovalEligibilityResult(
            eligible=eligible,
            reasons=reasons,
...
class BatchRenderGate:
    ...
    def evaluate(
        ...
        Args:
            ...
            policy: "strict" (default) or "permissive"
        ...
        if persona_diagnostics:
            ...
            if not automation_allowed:
                ...
                # Regulatory persona check - cannot be forced in strict mode
                if persona == "regulatory" and policy == "strict":
                    can_force = False
...
def get_prospect_state(
    ...
    Args:
        ...
        approved: Whether artifacts were approved for send
    ...
    if approved:
        return ProspectState.APPROVED_FOR_SEND
...
def get_promotion_state(
    ...
    """Backwards compatibility alias for get_prospect_state."""
    return get_prospect_state(status, validation_passed, sent, replied, promoted)


def format_eligibility_report(
    approval_result: ApprovalEligibilityResult,
    ...
    lines.append("")
    lines.append("APPROVAL (for sending):")
    status = "ELIGIBLE" if approval_result.eligible else "NOT ELIGIBLE"
    lines.append(f"  Status: {status}")

    if not approval_result.eligible:
        lines.append("  Reasons:")
        for reason in approval_result.reasons:
            lines.append(f"    - {reason}")

    if approval_result.warnings_present:
        lines.append(f"  Warnings: {', '.join(approval_result.warnings_present)}")

    lines.append(f"  Can force: {'Yes' if approval_result.can_force else 'No'}")
